...
def data_transforms(cfg):
    ...
    normalization_for_test = [
        transforms.Resize((cfg.data.input_size_test, cfg.data.input_size)),
        transforms.ToTensor(),
        transforms.Normalize(cfg.data.mean, cfg.data.std)
    ]
    ...
    test_preprocess = transforms.Compose(normalization_for_test)

    return train_preprocess, val_preprocess, test_preprocess
