...
def evaluate(cfg, model, test_dataset, estimator):
    ...
    log_file_path = "xception41_{}.log".format(cfg.solver.learning_rate)
    ...
    with open(log_file_path, 'a+') as log_file:
        ...
        confusion_matrix = estimator.get_conf_mat()
        for row in confusion_matrix:
            log_file.write(' ' + ' '.join([str(elem) for elem in row]) + '\n')
        log_file.write("\n")
    print('ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ')


def evaluate2(cfg, model, test_dataset, estimator):
    test_sampler = None
    test_loader = DataLoader(
        test_dataset,
        shuffle=(test_sampler is None),
